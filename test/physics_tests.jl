@testset "Physics tests" begin
    @testset "integrators improve accuracy with smaller Δt" for integrator! in [explicit_euler!, semi_implicit_euler!]
		...
		deltas = Float64[]
		for Δt in [1.0, 0.5, 0.2, 0.1, 0.05, 0.02, 0.01, 0.001]
			b = RigidBody(mass, force)
			for t in 0:Δt:t
				integrator!(b, Δt)
			end	
			push!(deltas, b.position.x - expected_distance)
		end
		# Deltas have decreasing value
	...
	@testset "total delta-v for Falcon 9" begin
		...
		fuel_mass1 = 411e3		# propellant mass first stage
		
		dry_mass2 = 4e3			# dry mass second stage
		fuel_mass2 = 107.5e3	# propellant mass second stage
		
		payload_mass = 22.8e3		# payload to LEO for F9
		
		total_mass1 = dry_mass1 + fuel_mass1 + 
		              dry_mass2 + fuel_mass2 + payload_mass
		
		
  		total_mass2 = dry_mass2 + fuel_mass2 + payload_mass
		...
		Δv₁ = delta_velocity(vₑ, total_mass1, total_mass1 - fuel_mass1)
		
		vₑ = exhaust_velocity(Isp_Vac2)
		Δv₂ = delta_velocity(vₑ, total_mass2, total_mass2 - fuel_mass2)
		Δv = Δv₁ + Δv₂
