...
"""
function thrust(r::Rocket) :: Float64
    @assert (r.engine.min_throttle <= r.throttle <= 1.0) || r.throttle == 0
    core_thrust = thrust(r.engine) * clamp(r.throttle, r.engine.min_throttle, 1.0)
    thrustdata = thrust.(r.sideboosters) :: Array{Float64}
    booster_thrust = sum(thrustdata)
    core_thrust + booster_thrust
end

thrust(engine::SingleEngine)   :: Float64  = engine.thrust
thrust(cluster::EngineCluster) :: Float64  = thrust(cluster.engine) * cluster.count
...
function mass(r::Rocket)
    booster_mass = sum(mass.(r.sideboosters)) :: Float64
